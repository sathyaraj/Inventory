...
constructor(private fb: FormBuilder,private router: Router,private masterService:Master,private cdr: ChangeDetectorRef,private route: ActivatedRoute,private adminService:Adminmaster) {}
...
loadMasters() {
  this.adminService.getServiceitemList().subscribe(res => {
      console.log('API Response:', res);
    console.log('First Record:', (res as any[])[0]);
...
applyFilters() {
  ...
  this.filteredItems = this.items.filter(item => {
    ...
    // GLOBAL SEARCH
    const globalMatch =
      (item?.serviceSet || '').toLowerCase().includes(globalText) ||
      (item?.serviceCode || '').toLowerCase().includes(globalText) ||
      (item?.serviceName || '').toLowerCase().includes(globalText) ||
      ...
      (item?.orderUnit || '').toLowerCase().includes(globalText) ||
      (item?.issueUnit || '').toLowerCase().includes(globalText);
...
confirmDelete() {
  ...
  this.adminService.servicedeleteItem(this.deleteId).subscribe({

    next: (res:any) => {

      this.messageTitle = 'Success';
      this.messageText = 'Delete successfully';
...
openItem(id: number) {
  this.router.navigate(['/admin/inventoryservice/service-create', id]);
...
downloadItems() {
  ...
  this.adminService.exportItems(this.fromDate, this.toDate)
    .subscribe((res: Blob) => {
...
fieldTitleMap: Record<string, string> = {
  ...
  ReceiptTolerance: 'Receipt Tolerance',
  OrderUnit: 'Order Unit',
  IssueUnit: 'Issue Unit',
  InspectOnReceipt: 'Inspect On Receipt',
  TaxExempt: 'Tax Exempt',
  Taxcode: 'Tax Code',
  CostCenter: 'Cost Center',
  MinimumServiceCost: 'Minimum Service Cost',
  MaximumServiceCost: 'Maximum Service Cost',
  LeadTimeDays: 'Lead Time Days',
  ActiveForPurchase: 'Active For Purchase',
  ActiveForWorkOrder: 'Active For Work Order',
  Prorate: 'Prorate',
