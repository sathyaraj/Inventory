...
import { LucideAngularModule,Trash,SquarePen,Info, Settings,MoveUp,ArrowDownNarrowWide,ArrowUpNarrowWide } from 'lucide-angular';
...
@Component({
  selector: 'app-costcodelist',
  imports: [CommonModule,LucideAngularModule, MessageBox],
  templateUrl: './costcodelist.html',
  styleUrl: './costcodelist.css',
})
...
createcostcode()
{
  this.router.navigate(['/admin/admin-control/costcode']);
}
...
fieldTitleMap: Record<string, string> = {
  ...
  costCodeNo: 'Cost code No',

  costCodeName: 'cost center Name',
...
getcostcode() {
  ...
  this.adminMaster.getcostcodelist().subscribe(res => {
...
editCostcode(item: any) {
console.log(item)
  this.router.navigate([
    '/admin/admin-control/costcode',
    item.id
  ]);
